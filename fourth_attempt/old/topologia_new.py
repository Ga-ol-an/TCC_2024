...
def customTopology():
    ...
    net.addLink(h3, s2)#, cls=TCLink, bw=10, delay='150ms', loss=1)#, jitter='30ms')
    
    net.addLink(h4, s3)#, cls=TCLink, bw=10, delay='150ms', loss=1)#, jitter='30ms')
    ...
    s1.cmd('xterm -e "ovs-vsctl set Bridge s1 stp_enable=true protocols=OpenFlow13"')
    s2.cmd('xterm -e "ovs-vsctl set Bridge s2 stp_enable=true protocols=OpenFlow13"')
    s3.cmd('xterm -e "ovs-vsctl set Bridge s3 stp_enable=true protocols=OpenFlow13"')
